    ...
    axios
      ...
      .catch(error => {
        if (error.response.status == 500) {
          toastr.error("There was an error trying get the appointment.");
        }
  ...
  methods: {
    saveAppointment: function() {
      axios
        ...
        .catch(error => {
          if (error.response.status == 500) {
            toastr.error("Appointment couldn't be edited due to an unexpected error.");
...
// @import 'admin-lte/tempusdominus-bootstrap-4/build/css/tempusdominus-bootstrap-4';
</style>
